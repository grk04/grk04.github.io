
gunashree.rajendra04@gmail.com | Seattle, Washington (open to relocate) | (206) 739-4232 | https://linkedin.com/in/gunashree04/ | https://github.com/grk04 

SUMMARY

As a technology professional with over 3 years of software development experience & 4 years of QA management, I offer a rich blend of expertise in developing & improving quality of consumer-facing cloud & enterprise products alongside project planning, and stakeholder collaboration. I am excited about the opportunity to apply my former experiences along with my academic exposure as a new graduate student. 

EDUCATION

  Masters of Science in Information Management - Data Science (Sep 2023 – Jun 2024)    
  University of Washington, Seattle, WA 	
  Relevant Coursework: Predictive Modeling and Machine Learning, AI, Python Programming, Principles of Product/Program Management, System Design & Data structures, Management & Strategic Leadership
  
SKILLS

  Languages (Proficient): Java, SQL, Spring Boot, Perl, Python, C++, JavaScript, React, Node, HTML, CSS,
  Languages (Familiar): C#, PhP
  Frameworks: Cucumber, Selenium, Gitlab, Mocha, Java Spring, TestNG, JUnit, Camel, Mockito, REST, JDK
  Tools: Jenkins, Jira, Gradle, HP ALM, MS Office, Docker, Linux, Kubernetes, Azure DevOps Board, Postman
  Databases: Oracle 11g/12c, MongoDB, NoSQL, Kafka, AWS EC2, PCF, GCP
  Relevant Skills: Cloud Infrastructure, Distributed Systems, Object-oriented design, CI/CD, Microsoft Azure, Product Strategy & Leadership, Presentations, Communication, Competitor Analysis, User Research, Agile Methodologies, User Acceptance Testing, Product Roadmap & Development
  
TECHNICAL PROJECTS

  Design Similarity Detector Alaska Challenge (Technologies: Python, Matplotlib, TensorFlow) Dec 2023 – Feb 2024
•	Developed a Convolutional Neural Network model for customers to upload design preferences, automating design validation to ensure only high-confidence designs proceed to production while enabling upcycling of existing textile waste.
•	Led a team of 4 in devising a strategy to reduce textile waste and promote sustainable fashion, culminating in an innovative ecosystem aligning customer preferences with environmental values
  
  Machine Learning Model (Technologies: Azure LLM, Python, C#) Sep 2023 – Dec 2023
•	Engineered a Python-based ML model to forecast Airbnb listing prices, resulting in a 20% improvement in prediction accuracy 
  
  LRG Rank Python Application (Technologies: Python, Shell, XML) Jul 2018 – Dec 2018
•	Designed and implemented a novel LRG(Large Regression) ranking model to prioritize testing efforts, significantly enhancing efficiency in identifying potential issues before merging transactions, resulting in smoother software integration processes
•	Validated the model's effectiveness using real-world data on culprit transactions, demonstrating its ability to identify high-risk areas and suggest improvements for future tuning and optimization, thereby enhancing software quality
•	Utilized advanced metrics and algorithms to assess the relevance of code and test files to transaction changes, creating a comprehensive scoring system that accurately ranks LRGs based on their connection with modified files  

PROFESSIONAL EXPERIENCE

  Senior Software Engineer, Wells Fargo (Bengaluru, India) Dec 2022 – Sep 2023
•	Implemented a ground-up automation framework to process Swift payments, reducing manual testing effort by 50%
•	Led sprint planning, conducted daily scrums, and led retrospectives to ensure effective Agile project management
•	Collaborated cross-functionally to integrate data streams involving 2 product owners, 1 BA, and 5+ client support teams
•	Managed JIRA bug tracking flow by prioritizing show-stopper issues & defining project scopes, deliverables, and timelines

  Software Engineer, Wells Fargo (Bengaluru, India) Nov 2020 – Dec 2022
•	Created 600 functional test cases using Mockito, JUnit, and 200 BDD/TDD test cases using Cucumber to increase coverage 
•	Executed usability & regression tests for a charting visualization engine on the Web browser and Android utilizing Selenium
•	Built a Rest API-based parser for testing service layer functionalities using Python & SQL, benefiting 8 developers
  
  Member of Technical Staff, Oracle (Bengaluru, India) Jul 2016 – Oct 2020
•	Designed and developed features from the ground up to provision virtualized computing resources, scaling, patching, backup & recovery processes using C++, Java, & SQL (Infrastructure as a Service (IaaS)
•	Developed wallet-certification functionality for autonomous data warehouse cloud offering using REST, Perl, & SQL
•	Implemented Jenkins Continous Integration/Deployment for Java virtual machine in Git repositories
•	Transitioned the existing platform to Microservices architecture using Perl & Python resulting in increased scalability
•	Optimized a distributed PTP app leveraging multithreading for concurrent execution saving over 80+ work hours monthly
                                               
    LEADERSHIP & DEVELOPMENT
    
•	Serving as a Reader/Grader at Fosters School of Business – Strategic Business Communication (Apr 2024 – Jun 2024) 
•	Spearheaded the role of a Senior Consultant at Fosters Business Group, Seattle, WA (Jan 2024 – Apr 2024)                                                                                                                                             



